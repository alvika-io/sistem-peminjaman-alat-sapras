...
<head>
    ...
    <style>
        ...
        /* Custom Checkbox Logic Visuals */
        .select-item-checkbox:checked + label {
            border-color: #10b981 !important;
            background-color: #f0fdf4;
        }

        .select-item-checkbox:checked + label .checkbox-box {
            background-color: #10b981;
            border-color: #10b981;
        }

        .select-item-checkbox:checked + label .checkbox-icon {
            opacity: 1;
            transform: scale(1);
        }
        ...
        /* Flatpickr Premium Emerald Styling */
        ...
        <main class="p-8 lg:p-12">
            <div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                ...
                <a href="{{ route('peminjam.peminjaman.create') }}" 
                   class="flex items-center gap-2 px-6 py-4 bg-white border-2 border-emerald-100 text-emerald-600 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-emerald-50 hover:border-emerald-200 transition-all shadow-sm shadow-emerald-100/50">
            ...
            @if($items->isEmpty())
                <div class="bg-white p-20 rounded-[3rem] border border-dashed border-gray-200 text-center">
                    ...
                    <p class="text-gray-400 text-sm mt-2 mb-8 text-balance">Mungkin kamu belum sempat memilih alat sapras yang ingin dipinjam.</p>
                    <a href="{{ route('peminjam.peminjaman.create') }}" class="inline-block px-8 py-4 bg-emerald-600 text-white font-black rounded-2xl uppercase text-[10px] tracking-widest hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-200">Cari Alat Sekarang</a>
                </div>
            @else
                <form action="{{ route('peminjam.peminjaman.store') }}" method="POST" id="form-checkout" class="form-confirm" data-message="Yakin ingin mengajukan peminjaman untuk alat yang Anda pilih?">
                    @csrf
                            ...
                            @foreach($items as $index => $item)
                                <div class="relative group">
                                    <input type="checkbox" id="check{{ $index }}" 
                                           class="select-item-checkbox absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" 
                                           onchange="updateSelection({{ $index }})">
                                    
                                    <label for="check{{ $index }}" class="flex items-center justify-between p-6 bg-white border-2 border-transparent rounded-[2rem] transition-all duration-300 shadow-sm group-hover:border-emerald-100">
                                        <div class="flex items-center gap-5">
                                            <div class="checkbox-box w-6 h-6 border-2 border-gray-200 rounded-lg flex items-center justify-center flex-shrink-0 transition-all">
                                            ...
                                            <div class="w-16 h-16 rounded-2xl bg-gray-50 overflow-hidden flex-shrink-0 border border-gray-100">
                                                ...
                                                @else
                                                    <div class="w-full h-full flex items-center justify-center bg-emerald-50 text-emerald-600 font-black">{{ substr($item->alat->nama, 0, 1) }}</div>
                                                @endif
                                            </div>
                                            <div>
                                                <h4 class="font-black text-gray-900 leading-tight">{{ $item->alat->nama }}</h4>
                                                <p class="text-xs font-bold text-emerald-600 mt-1">{{ $item->jumlah }} Unit</p>
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex items-center gap-4">
                                            ...
                                            <button type="button" onclick="konfirmasiHapusKeranjang({{ $item->id }})" class="relative z-20 p-3 text-gray-300 hover:text-red-500 transition-colors">
                        ...
                        <div class="space-y-6">
                            <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 sticky top-8">
                                ...
                                <div class="mt-8 pt-8 border-t border-gray-50 text-center">
                                    <button type="submit" id="btn-submit" disabled class="w-full py-5 bg-gray-200 text-gray-400 font-black rounded-3xl uppercase tracking-widest text-[10px] transition-all cursor-not-allowed">
                                        Ajukan Peminjaman
                                    </button>
                                    <p class="text-[9px] text-gray-400 mt-4 font-bold uppercase tracking-widest">Pilih minimal 1 alat</p>
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    function updateSelection(index) {
        ...
        const anyChecked = document.querySelectorAll('.select-item-checkbox:checked').length > 0;
        if (anyChecked) {
            btnSubmit.disabled = false;
            btnSubmit.classList.remove('bg-gray-200', 'text-gray-400', 'cursor-not-allowed');
            btnSubmit.classList.add('bg-emerald-600', 'text-white', 'hover:bg-emerald-700', 'shadow-xl', 'shadow-emerald-200');
        } else {
            btnSubmit.disabled = true;
            btnSubmit.classList.add('bg-gray-200', 'text-gray-400', 'cursor-not-allowed');
            btnSubmit.classList.remove('bg-emerald-600', 'text-white', 'hover:bg-emerald-700', 'shadow-xl', 'shadow-emerald-200');
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const commonConfig = {
            locale: "id", altInput: true, altFormat: "d F Y", dateFormat: "Y-m-d", minDate: "today", static: true,
        };
