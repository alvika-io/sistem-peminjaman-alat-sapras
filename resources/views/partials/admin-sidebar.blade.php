...
<aside id="sipras-sidebar" class="flex flex-col w-72 h-screen bg-white dark:bg-slate-900 border-r border-gray-100 dark:border-slate-800 shadow-sm overflow-y-auto transition-colors duration-300">
    ...
    <div class="flex-1 px-4 py-4 overflow-y-auto">
        ...
        <nav class="space-y-2">
            ...
            <p class="px-4 mb-4 text-[11px] font-black text-gray-300 dark:text-slate-700 uppercase tracking-widest">Settings</p>

            <a href="{{ route('profile.edit') }}" 
               class="sidebar-transition flex items-center gap-4 px-4 py-3.5 rounded-2xl font-bold text-sm {{ request()->routeIs('profile.*') ? 'nav-item-active shadow-sm' : 'text-gray-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400' }}">
    ...
    <div class="p-6 mt-auto">
        <div class="bg-gray-50 dark:bg-slate-800 rounded-[2rem] p-4 flex items-center gap-3 border border-gray-100 dark:border-slate-700 transition-colors">
            <div class="flex-shrink-0 w-10 h-10 rounded-full bg-blue-600 border-2 border-white dark:border-slate-900 flex items-center justify-center text-white font-black text-xs shadow-sm uppercase">
                {{ substr(Auth::user()->name, 0, 2) }}
            </div>
            <div class="overflow-hidden">
                <p class="text-[12px] font-black text-gray-800 dark:text-slate-100 truncate uppercase tracking-tighter">{{ Auth::user()->name }}</p>
