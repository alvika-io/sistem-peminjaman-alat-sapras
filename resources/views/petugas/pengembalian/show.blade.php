                        ...
                        <div class="flex gap-4">
                            ...
                            @if($pengembalian->denda > 0 && $pengembalian->denda_status === 'belum_dibayar')
                                <form action="{{ route('petugas.pengembalian.updateStatusDenda', $pengembalian->id) }}" method="POST" class="flex-1">
                                    @csrf
                                    @method('PUT')
                                    <button type="submit" class="w-full py-4 bg-green-500 hover:bg-green-600 text-white font-black rounded-2xl shadow-xl shadow-green-100 transition-all flex items-center justify-center gap-3 uppercase tracking-widest text-xs transform hover:-translate-y-1 active:scale-95">
